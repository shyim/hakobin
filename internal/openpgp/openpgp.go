...
// EnsureActiveUsable returns an error when the active signing key exists but has
// expired, so mutating commands do not publish freshly-signed metadata that
// clients will reject. The check can be overridden by setting
// HAKOBIN_ALLOW_EXPIRED_KEY to a truthy value (for deliberate recovery).
func (s *SigningKeys) EnsureActiveUsable() error {
	if s.Active == nil || s.Active.Expiration == nil {
		return nil
	}
	if !s.Active.Expiration.Before(time.Now()) {
		return nil
	}
	if allowExpiredKey() {
		return nil
	}
	return fmt.Errorf("active signing key %s expired on %s; refusing to sign (set HAKOBIN_ALLOW_EXPIRED_KEY=1 to override)",
		s.Active.KeyID, s.Active.Expiration.Format("2006-01-02"))
}

func allowExpiredKey() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HAKOBIN_ALLOW_EXPIRED_KEY")))
	switch v {
	case "1", "true", "yes", "on":
		return true
	}
