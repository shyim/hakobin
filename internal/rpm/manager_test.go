...
func TestWriteMetadataPrunesOwnFilesButPreservesForeignRepodata(t *testing.T) {
	rm, store := testManager()
	ctx := context.Background()
	dir := t.TempDir()
	require.NoError(t, rm.Init(ctx, &RpmInitRequest{Repo: "stable", Arch: "x86_64"}))

	// Simulate a foreign metadata producer dropping a sqlite db and an
	// updateinfo file that Hakobin did not create.
	foreign := []string{
		"rpm/stable/x86_64/repodata/deadbeef-primary.sqlite.bz2",
		"rpm/stable/x86_64/repodata/cafebabe-updateinfo.xml.gz",
	}
	for _, k := range foreign {
		require.NoError(t, store.UploadBytes(ctx, k, []byte("x"), "application/octet-stream"))
	}

	// Record the primary file Hakobin published at init.
	var initPrimary string
	for _, k := range store.Keys() {
		if bytes.Contains([]byte(k), []byte("-primary.xml.gz")) {
			initPrimary = k
		}
	}
	require.NotEmpty(t, initPrimary)

	// Upload a package: this regenerates metadata with a new primary file.
	path := makeRpm(t, dir, "demo", "1.0.0", "1.el9", "x86_64")
	require.NoError(t, rm.Upload(ctx, &RpmUploadRequest{RpmFiles: []string{path}, Repo: "stable", Arch: "x86_64"}))

	// Hakobin's old primary must be pruned...
	_, ok := store.Body(initPrimary)
	assert.False(t, ok, "old Hakobin primary should be pruned after regeneration")

	// ...but the foreign repodata must be preserved.
	for _, k := range foreign {
		_, ok := store.Body(k)
		assert.True(t, ok, "foreign repodata %s must not be deleted", k)
	}
}
