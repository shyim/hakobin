...
func TestS3StoreLockAcquireReleaseAndConflict(t *testing.T) {
	...
	lock, err := store.AcquireLock(ctx, "deb/.lock", "owner-a", 300, 1000)
	require.NoError(t, err)
	...
	defer cancel()
	_, err = store.AcquireLock(blocked, "deb/.lock", "owner-b", 300, 1001)
	require.Error(t, err)

	// After release, re-acquire succeeds immediately.
	require.NoError(t, store.ReleaseLock(ctx, lock))
	lock2, err := store.AcquireLock(ctx, "deb/.lock", "owner-b", 300, 1002)
	require.NoError(t, err)
...
func TestS3StoreLockStealsExpired(t *testing.T) {
	...
	// Past the TTL, the expired lock is stolen.
